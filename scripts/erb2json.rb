...
def frontmost_application(type, app_aliases)
  ...
  microsoft_excel_bundle_identifiers = [
    '^com\.microsoft\.Excel$',
  ]
  ...
  app_aliases.each do |app_alias|
    ...
    when 'microsoft_powerpoint'
      bundle_identifiers.concat(microsoft_powerpoint_bundle_identifiers)

    when 'microsoft_excel'
      bundle_identifiers.concat(microsoft_excel_bundle_identifiers)
